...
export function parseBody<T extends z.ZodType>(schema: T, body: unknown): z.infer<T> {
  ...
  if (!result.success) throw badRequest('api.invalidBody', undefined, result.error.issues)
  return result.data
}

export function parseQuery<T extends z.ZodType>(schema: T, query: unknown): z.infer<T> {
  const result = schema.safeParse(query)
  if (!result.success) throw badRequest('api.invalidQuery', undefined, result.error.issues)
  return result.data
}
...
export function requirePrincipalId(id: string | undefined): string {
  if (id === undefined) throw unauthenticated('api.unauthenticated')
  return id
}
