...
const stubRhiza = (name: string, api: unknown): GerminatedRhiza => ({
  name,
  manifest: { kind: 'rhiza', name, septum: '^1.0' },
...
function harness(options: {
  ...
  const hypha = {
    name: 'console',
    config: {},
    manifest: { kind: 'hypha' as const, name: 'console', septum: '^1.0', capabilities: options.capabilities ?? [] },
    instance: {
  ...
  const enzyme = {
    ...
    scopes: [],
    manifest: { kind: 'enzyme' as const, name: 'media', septum: '^1.0', commands: options.commands },
    instance: {
      handlers: {
...
describe('the four refusal callbacks, threaded with the locale bus.ts resolved', () => {
  it("passes the reader's locale, not the default, to each of onUnrouted/onDenied/onUnsupported/onOutOfContext", async () => {
    ...
    const hypha = {
      name: 'console', config: {},
      manifest: { kind: 'hypha' as const, name: 'console', septum: '^1.0', capabilities: [] },
      instance: {
    ...
    const enzyme = {
      ...
      manifest: {
        kind: 'enzyme' as const, name: 'media', septum: '^1.0',
        commands: [
