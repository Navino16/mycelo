...
import { GuidedStart, outstandingSteps } from '../components/GuidedStart.tsx'
...
export function Overview(): React.JSX.Element {
  ...
  // A step nobody could confirm is not "done": null keeps the ordinary tiles up rather
  // than guessing, and the alert below is what tells the operator the count is missing.
  const steps = counts === null ? [] : outstandingSteps(counts)
  ...
  return (
    <div className="space-y-6">
      ...
      {steps.length > 0 && counts !== null
        ? <GuidedStart counts={counts} />
        : (
          <>
            {unreadable && (
